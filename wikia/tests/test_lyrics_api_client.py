...
def test_get_song(lyrics_library, song_title1):
    ...
    assert song.lyrics.text == lyricwikia.get_lyrics(
        song_title1.artist, song_title1.title)
...
def test_get_all_songs_by_artist():
    all_songs = wikia.lyrics_api_client.find_all_songs_by_artist(
        'Led Zeppelin')
...
def test_lyrics_not_found(lyrics_library):
    assert lyrics_library.get_random_song_by_artist(
        'Emiliano Menéndez') == NullSong()


def test_instrumental_song(lyrics_library):
    song_title = SongTitle('Deep Forest', 'Boheme')
    assert lyrics_library.get_song(
        song_title) == InstrumentalSong('Deep Forest', 'Boheme')
